...
Revision ID: b131159a37c9
Revises: d4e24f21ab13
Create Date: 2021-10-20 16:32:17.656637
...
revision = 'b131159a37c9'
down_revision = 'd4e24f21ab13'
branch_labels = None
depends_on = None
...
def upgrade():
    ...
    connection.execute(
        """
            ...
            GRANT ALL ON TABLE audit.logged_actions TO dev_users;
            GRANT ALL ON TABLE audit.logged_actions TO postgres;
            GRANT ALL ON TABLE audit.logged_actions TO api_server; 
            
            SELECT audit.audit_table('activities');
            SELECT audit.audit_table('inventories');
...
def downgrade():
    ...
    connection.execute(
        """
        DROP TRIGGER IF EXISTS audit_trigger_row on activities;
        DROP TRIGGER IF EXISTS audit_trigger_stm on activities;

        DROP TRIGGER IF EXISTS audit_trigger_row on inventories;
        DROP TRIGGER IF EXISTS audit_trigger_stm on inventories;

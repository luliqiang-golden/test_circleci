...
class ExternalOrderUpdateStatus(ActivityHandler):
    """
        Action to update the status of the external order
            ...
            if (args['to_status'] == 'cancelled'):
                ...
                if (order_items):
                    for order_item in order_items:
                        update_resource_attribute_into_db('skus', order_item['sku_id'], 'reserved', order_item['reserved_quantity'] - order_item['quantity'])
                        activities_list = cls.get_order_item_map_to_lot_item_activity_id(args['organization_id'], order_item['id'])
                        if (activities_list):
                            for activity in activities_list:
                                delete_from_db("activities", activity['id'], args['organization_id'])    


            bulk_update_result = bulk_update_column_into_db("order_items", args["organization_id"], 'status', args["to_status"], [('order_id', '=', args['order_id']), ('status', '!=', 'cancelled')])
